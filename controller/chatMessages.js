...
async function chatMessagesGet(req, res) {
    try{
        ...
        if (!chatMessageList[0]) {
            return res.status(400).send({ result: "false", message: "해당 채팅방의 메시지가 존재하지 않습니다.", titleRoom : titleRoom.postTitle});
        } else {
            return res.status(200).send({ chatMessageList: chatMessageList, titleRoom : titleRoom.postTitle });
        }
    } catch (err) {
