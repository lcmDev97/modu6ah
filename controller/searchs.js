...
async function searchAll(req, res) {
    try {
        ...
        const resultsInRecruit = await recruitPost
            .find({ $or: options }, { bookmarkUsers: 0, bookmarkStatus: 0 })
            .sort({ recruitPostId: -1 });
        const resultsInPlace = await placePost
            .find({ $or: options }, { bookmarkUsers: 0, bookmarkStatus: 0 })
            .sort({ placePostId: -1 });
        const resultsInReview = await reviewPost
            .find({ $or: options }, { bookmarkUsers: 0, bookmarkStatus: 0 })
            .sort({ reviewPostId: -1 });
